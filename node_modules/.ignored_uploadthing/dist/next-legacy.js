...
var fileCountLimitHit = (files, routeConfig) => {
  ...
  const counts = {};
  files.forEach((file) => {
    const type = (0, import_shared.getTypeFromFileName)(
      file,
...
var buildRequestHandler = (opts) => {
  return async (input) => {
    ...
    const params = new URL(req.url ?? "", (0, import_shared.getUploadthingUrl)()).searchParams;
    ...
    if (!slug)
      return new import_shared.UploadThingError({
        code: "BAD_REQUEST",
    ...
    switch (actionType) {
      case "upload": {
        ...
        let parsedConfig;
        try {
          parsedConfig = (0, import_shared.fillInputRouteConfig)(
        ...
        const uploadthingApiResponse = await fetch(
          (0, import_shared.generateUploadThingURL)("/api/prepareUpload"),
          {
            method: "POST",
            body: JSON.stringify({
              files,
              ...
              metadata,
              callbackUrl: (config == null ? void 0 : config.callbackUrl) ?? (0, import_shared.getUploadthingUrl)(),
              callbackSlug: slug
            }),
            headers: {
      ...
      case "failure": {
        ...
        const uploadthingApiResponse = await fetch(
          (0, import_shared.generateUploadThingURL)("/api/failureCallback"),
          {
            method: "POST",
            body: JSON.stringify({
              fileKey
        ...
        try {
          uploadable._def.onUploadError({
            error: new import_shared.UploadThingError({
              code: "UPLOAD_FAILED",
...
var buildPermissionsInfoHandler = (opts) => {
  return () => {
    ...
    const permissions = Object.keys(r).map((k) => {
      const route = r[k];
      const config = (0, import_shared.fillInputRouteConfig)(route._def.routerConfig);
      return {
        slug: k,
        config
      };
...
var createNextPageApiHandler = (opts) => {
  ...
  return async (req, res) => {
    ...
    if (response instanceof import_shared2.UploadThingError) {
      res.status((0, import_shared2.getStatusCodeFromError)(response));
      res.setHeader("x-uploadthing-version", UPLOADTHING_VERSION);
