...
var uploadFilesInternal = async (data, opts) => {
  ...
  const res = await fetch((0, import_shared.generateUploadThingURL)("/api/uploadFiles"), {
    method: "POST",
    headers: {
  ...
  if (!res.ok) {
    throw import_shared.UploadThingError.fromResponse(res);
  }
  const clonedRes = res.clone();
  const json = await res.json();
  if ("error" in json) {
    throw import_shared.UploadThingError.fromResponse(clonedRes);
  }
  const uploads = await Promise.allSettled(
    data.files.map(async (file, i) => {
      ...
      if (!s3res.ok) {
        await fetch((0, import_shared.generateUploadThingURL)("/api/failureCallback"), {
          method: "POST",
          body: JSON.stringify({
            fileKey: fields.key
          }),
      ...
      await (0, import_shared.pollForFileData)(key);
      return {
        key,
        url: fileUrl,
        name: file.name,
  ...
  return uploads.map((upload) => {
    ...
    const error = import_shared.UploadThingError.toObject(reason);
...
var deleteFiles = async (fileKeys) => {
  ...
  if (!Array.isArray(fileKeys))
    fileKeys = [fileKeys];
  const res = await fetch((0, import_shared2.generateUploadThingURL)("/api/deleteFile"), {
    method: "POST",
    cache: "no-store",
    headers: {
...
var getFileUrls = async (fileKeys) => {
  ...
  if (!Array.isArray(fileKeys))
    fileKeys = [fileKeys];
  const res = await fetch((0, import_shared2.generateUploadThingURL)("/api/getFileUrl"), {
    method: "POST",
    cache: "no-store",
    headers: {
      "Content-Type": "application/json",
...
var listFiles = async () => {
  guardServerOnly();
  const res = await fetch((0, import_shared2.generateUploadThingURL)("/api/listFiles"), {
    method: "POST",
    cache: "no-store",
    headers: {
      "Content-Type": "application/json",
...
var renameFile = async (updates) => {
  ...
  if (!Array.isArray(updates))
    updates = [updates];
  const res = await fetch((0, import_shared2.generateUploadThingURL)("/api/renameFile"), {
    method: "POST",
    cache: "no-store",
    headers: {
      "Content-Type": "application/json",
...
var fileCountLimitHit = (files, routeConfig) => {
  ...
  const counts = {};
  files.forEach((file) => {
    const type = (0, import_shared3.getTypeFromFileName)(
      file,
...
var conditionalDevServer = async (fileKey) => {
  if (process.env.NODE_ENV !== "development")
    return;
  const fileData = await (0, import_shared3.pollForFileData)(
    fileKey,
    async (json) => {
...
var buildRequestHandler = (opts) => {
  return async (input) => {
    ...
    const params = new URL(req.url ?? "", (0, import_shared3.getUploadthingUrl)()).searchParams;
    ...
    switch (actionType) {
      case "upload": {
        ...
        const uploadthingApiResponse = await fetch(
          (0, import_shared3.generateUploadThingURL)("/api/prepareUpload"),
          {
            method: "POST",
            body: JSON.stringify({
              files,
              ...
              metadata,
              callbackUrl: (config == null ? void 0 : config.callbackUrl) ?? (0, import_shared3.getUploadthingUrl)(),
              callbackSlug: slug
            }),
            headers: {
              "Content-Type": "application/json",
      ...
      case "failure": {
        ...
        const uploadthingApiResponse = await fetch(
          (0, import_shared3.generateUploadThingURL)("/api/failureCallback"),
          {
            method: "POST",
            body: JSON.stringify({
              fileKey
  ...
  const POST = async (req) => {
    ...
    if (response instanceof import_shared4.UploadThingError) {
      ...
      return new Response(JSON.stringify(formattedError), {
        status: (0, import_shared4.getStatusCodeFromError)(response),
        headers: {
          "x-uploadthing-version": UPLOADTHING_VERSION
        }
      });
